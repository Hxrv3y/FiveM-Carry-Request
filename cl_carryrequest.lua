...
RegisterCommand("carry",function(source, args)
    if not carry.InProgress then
        ...
        if closestPlayer then
            ...
            if targetSrc ~= -1 then
                TriggerServerEvent("CarryRequest", targetSrc)
            else
...
RegisterNetEvent("Harv:StartCarry")
AddEventHandler("Harv:StartCarry", function(targetSrc)
...
RegisterNetEvent("Harv:CarryTargetAsk")
AddEventHandler("Harv:CarryTargetAsk", function(senderSrc)
    carryrequest = true
    Citizen.CreateThread(function()
        while carryrequest do
            if IsControlJustPressed(1, 83) then
                TriggerServerEvent("Harv:CarryAccepted",senderSrc)
                carryrequest = false
            elseif IsControlJustPressed(1, 84) then
                TriggerServerEvent("Harv:CarryDeclined",senderSrc)
                carryrequest = false
            end
            Wait(0)
        end
...
RegisterNetEvent("CarryNotify", drawNativeNotification)